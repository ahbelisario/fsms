  ...
  try {
    const [records, count] = await Promise.all([
      new Promise((resolve, reject) => {
        fsms_pool.query('SELECT * FROM income_types', (err, rows) => {
          if (err) reject(err);
          else resolve(rows);
        });
      }),
      new Promise((resolve, reject) => {
        fsms_pool.query('SELECT COUNT(*) AS total_rows FROM income_types', (err, rows) => {
          if (err) reject(err);
...
router.put('/:id', (req, res) => {
  ...
  fsms_pool.query(
    'UPDATE income_types SET name = ?, description = ? WHERE id = ?',
